...
uv run provide_role --duration 3600
```
...
- `template/provide_role.py` - AWS role provision script with MFA support (run via `uv run provide_role`)
...
- `.aws-dev/` - Temporary AWS credentials for dev container (gitignored)
  - Generated by `uv run provide_role`
...
- **AWS Credentials**: Optional - use `uv run provide_role` on host to provide AWS access to dev container (MFA required, updates live)
